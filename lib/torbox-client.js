...
async function addTorrent({
  ...
  try {
    if (magnet && !file) {
      ...
      // Native File upload via Form-Data with explicit binary handling
      const form = new FormData();
      
      const bytes = Buffer.isBuffer(file) ? file : (file instanceof Uint8Array ? Buffer.from(file) : null);
      if (!bytes) throw new Error('Érvénytelen fájl formátum az nCore-tól');

      form.append('file', bytes, {
        filename: name || 'movie.torrent',
        contentType: 'application/x-bittorrent'
      });
      
      form.append('allow_zip', allowZip ? 'true' : 'false');
      form.append('seed', '3'); // 3 = TorBox prevents Hit'n'Run (no seeding)

      // Using manual fetch because apiCall stringifies form boundary incorrectly
...
async function checkCached({ apiKey, infoHashes }) {
  ...
  try {
    ...
    const cached = new Set((Array.isArray(data?.data) ? data.data : []).map((item) => String(item?.hash || item?.info_hash || item?.infoHash || item?.torrent_hash || item || '').toLowerCase()).filter(Boolean));
