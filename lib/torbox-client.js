...
const RESOLVE_POLL_MS = Math.max(1000, Number(process.env.TORBOX_RESOLVE_POLL_MS || 2500) || 2500);
...
function isTorrentReady(torrent) {
  ...
  // TorBox: cached/completed = azonnal használható
  if (state === "cached" || state === "completed") return true;

  // A download_present + download_finished kombináció is kész torrentet jelent.
  if (torrent.download_present === true && torrent.download_finished === true) {
    return true;
  }
...
async function addTorrent({
  ...
  debugLog('torbox-create-input-fix', {
  ...
  try {
    if (magnet && !file) {
      // Torrentio pattern / magnet links (cached fallback)
      ...
      // Native .torrent upload via Node 22 FormData
      const form = new globalThis.FormData();

      const bytes = Buffer.isBuffer(file)
        ? file
        : (file instanceof Uint8Array ? Buffer.from(file) : null);

      if (!bytes) throw new Error("Érvénytelen .torrent fájl az nCore-tól");

      const blob = new Blob([bytes], {
        type: "application/x-bittorrent"
      });

      form.append("file", blob, name || "torrent.torrent");
      form.append("allow_zip", allowZip ? "true" : "false");
      form.append("seed", "3");

      const torboxResponse = await fetch(`${API_BASE}/v1/api/torrents/createtorrent`, {
        method: "POST",
        headers: {
          authorization: `Bearer ${apiKey}`
        },
      ...
      const text = await torboxResponse.text();
      let data;
      try { data = JSON.parse(text); } catch { data = { raw: text }; }

      if (!torboxResponse.ok || data.success === false) {
        throw new Error(`TorBox API hiba: ${JSON.stringify(data)}`);
      }
...
async function checkCached({ apiKey, infoHashes }) {
  ...
  try {
    ...
    console.error("[TORBOX-CHECKCACHED-RAW]", JSON.stringify({
      method: "POST",
      hashes,
      response: data
    }));
    ...
    const cached = new Set(
      (Array.isArray(data?.data) ? data.data : [])
        .map((item) => String(item?.hash || item?.info_hash || item?.infoHash || item?.torrent_hash || "").toLowerCase()).filter(Boolean)
    );

    return new Map(hashes.map((h) => [h, cached.has(h)]));
  } catch (err) {
    console.error("[TORBOX-CHECKCACHED-POST-ERROR]", JSON.stringify({
    ...
    try {
      ...
      console.error("[TORBOX-CHECKCACHED-GET-ERROR]", JSON.stringify({
        message: err2?.message || String(err2),
        status: err2?.status || null,
        data: err2?.data || null
      }));
...
async function resolveLink({
  ...
  const waitMs = Math.max(1000, Number(maxWaitMs) || 15000);
  ...
  // Poll until torrent is downloadable or timeout. This avoids immediate 409 loops.
  while (Date.now() <= deadline) {
    ...
    await sleep(Math.min(RESOLVE_POLL_MS, remainingMs));

    torrent = findTorrent(await getMyTorrents({ apiKey }), hash);
    if (torrent && isTorrentError(torrent)) {
      const torrentId = getTorrentId(torrent);
      if (torrentId) {
        try {
