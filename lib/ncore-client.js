...
async function loginAndSearch({ username, password, query }) {
  ...
  try {
    // First try the precise IMDb search.
    rows = await fetchAllSearchRows({
      cookie,
      imdbId: parsedQuery.imdbId,
      searchType: 'imdb',
    });

    // If IMDb search returns nothing, fall back to title search.
    if (!rows.length && parsedQuery.imdbId) {
      const title = await resolveImdbTitle(parsedQuery.imdbId);

      if (title) {
        console.error('[NCORE-TITLE-FALLBACK]', {
          imdbId: parsedQuery.imdbId,
          title,
        });

        const fallbackRows = await fetchAllSearchRows({
          cookie,
          title,
          searchType: 'name',
        });
        ...
        rows = fallbackRows.filter((row) =>
          isRelevantTitleSearchRow(row, title)
        );

        console.error('[NCORE-TITLE-FILTER]', {
          title,
          before: fallbackRows.length,
          after: rows.length,
        });

        searchType = 'name';
      } else {
        console.error('[NCORE-TITLE-FALLBACK] IMDb title could not be resolved', {
          imdbId: parsedQuery.imdbId,
        });
      }
    }
  ...
  const enriched = await mapLimit(
    rows,
    ENRICH_CONCURRENCY,
    async (row) => enrichRow({ row, cookie, parsedQuery })
  );

  const out = enriched.filter(
    (item) => item && (item.magnet || item.downloadUrl)
  );
...
async function resolveImdbTitle(imdbId) {
  ...
  const endpoints = [
    ...
    `https://v3-cinemeta.strem.io/meta/movie/${encodeURIComponent(id)}.json`,
  ];
...
function isRelevantTitleSearchRow(row, title) {
  const target = normalizeSearchTitle(title);
  const candidate = normalizeSearchTitle(row?.title);
  ...
  if (!words.length || candidateWords.length < words.length) return false;

  for (let i = 0; i <= candidateWords.length - words.length; i += 1) {
    if (words.every((word, index) => word === candidateWords[i + index])) {
      return true;
    }
  }
...
async function fetchAllSearchRows({ cookie, imdbId = '', title = '', searchType = 'imdb' }) {
  const rows = [];
  let page = 1;
  let lastPage = 1;
  ...
  do {
    ...
    for (const format of SEARCH_QUERY_FORMATS) {
      ...
      if (searchType === 'name' && title) {
        searchUrl =
          `${NCORE_BASE}/torrents.php?mire=${encodeURIComponent(title)}` +
          `&miben=name&tipus=all_own&tags=&miszerint=seeders&hogyan=DESC&oldal=${page}${suffix}`;
      } else {
        searchUrl =
          `${NCORE_BASE}/torrents.php?mire=${encodeURIComponent(imdbId)}` +
          `&miben=imdb&miszerint=seeders&hogyan=DESC&oldal=${page}${suffix}`;
      }
...
async function enrichRow({ row, cookie, parsedQuery }) {
  ...
  try {
    ...
    if (isSeriesRequest) {
      const picked = findSeriesVideoFile(torrent.videoFiles || [], parsedQuery.season, parsedQuery.episode, row.title);
...
function findSeriesVideoFile(files, season, episode, torrentTitle = '') {
  const targetSeason = Number(season);
  const targetEpisode = Number(episode);

  const seasonMatch = String(torrentTitle || '').match(/\bS(\d{1,2})(?:\b|[^\d])/i);
  const torrentSeason = seasonMatch ? Number(seasonMatch[1]) : null;
  ...
  for (const file of files || []) {
    ...
    if (!episodes.includes(targetEpisode)) continue;

    if (seasons.includes(targetSeason)) {
      return file;
    }

    if (seasons.length === 0 && torrentSeason === targetSeason) {
      return file;
    }
  }
